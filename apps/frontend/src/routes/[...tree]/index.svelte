<script lang="ts">
    import {page} from '$app/stores';
    import Link from "../../components/Link.svelte";

    const links = [
        {text: 'teste', url: 'https://google.com'},
        {text: 'teste2'}
    ];

</script>

<div id="container" class="flex flex-col items-center justify-between px-4 py-8 bg-amber-300">

    <div class="flex flex-col  items-center space-y-3 w-full min-h-screen max-w-screen-sm">

        <div class="flex flex-col justify-center items-center space-y-3">
            <img
                  class="rounded-full block w-24 "
                  src="data:image/png;base64,"
                  alt="nada"
            >
            <div class="font-mont">{$page.params.tree || '@allanvobraun'}</div>
        </div>
        <div id="links" class="self-stretch space-y-6 mt-4 flex flex-col">
            {#each links as link}
                <Link text="{link.text}" linkStyle="rounded-square" url="{link.url}"/>
            {/each}
        </div>

    </div>

</div>
<style>
    #container {
        position: relative;
        padding-bottom: 0;
        min-height: 100vh;
        overflow-x: hidden;
        width: 100%;
        height: 100%;
    }

</style>